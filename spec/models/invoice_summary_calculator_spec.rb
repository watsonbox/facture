...
describe InvoiceSummaryCalculator do
  describe '.subtotals_by_month' do
    let!(:invoice1) { create(:invoice, :with_line_items, :date => Time.now) }     # 49.95 €
    let!(:invoice2) { create(:invoice, :with_line_items, :date => Time.now) }     # 49.95 €
    let!(:invoice3) { create(:invoice, :with_line_items, :date => 1.month.ago) }  # 49.95 €
